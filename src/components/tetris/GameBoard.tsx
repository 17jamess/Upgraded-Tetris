    ...
    return displayBoard.map((row, y) => (
      <div key={y} className="flex">
        {row.map((cell, x) => (
          <div
            key={`${y}-${x}`}
            className="w-7 h-7 border border-gray-700/30 transition-all duration-75"
            style={{
      ...
      {gameOver && (
        <div className="absolute inset-0 bg-black/80 flex items-center justify-center rounded-lg">
          <div className="text-center text-white">
            ...
            <button onClick={onStartGame} className="bg-purple-600 hover:bg-purple-700 px-4 py-2 rounded">
              Play Again
            </button>
          </div>
        </div>
